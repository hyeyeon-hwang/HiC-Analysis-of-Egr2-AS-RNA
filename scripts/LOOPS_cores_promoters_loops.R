...
# Find COREs-promoters and promoters-COREs loops
...
# Test with each loop set
# loops <- loops_diff_lost 
# loops <- loops_diff_gain
# loops <- loops_diff_up_notgain
# loops <- loops_diff_down_notlost
# loops <- loops_sig_notdiff_all
loops <- loops_all
...
indices.p.a1 <- GenomicRanges::findOverlaps(
     ...
     type='within', # promoters 2200 within loop anchor1 10000
     select='all', ignore.strand=TRUE)
indices.p.a2 <- GenomicRanges::findOverlaps(
     gr_promoters, gr_loops_a2,
     type='within', # promoters 2200 within loop anchor2 10000
     select='all', ignore.strand=TRUE)

# There can be repeats of a1 start coordinates (coords) since the end coords differ
# Loop coords may repeat since 1 gene can map to multiple promoters (names col)
# The same promoter can be in the same a1 or a2 coord (different myid_loop but same startJ)
# 311 -> 259 unique promoters
# prom.in.Loops.a1.promoters <- gr_promoters[indices.p.a1@from]
...
# table(prom.in.Loops.a1.promoters$myid_promoter)[unique(prom.in.Loops.a1.promoters$myid_promoter)]
# a1 coord in prom.in.Loops.a1.loops is same for different (adjacent) loops, 
#       so the same promoter will map to multiple loops with the same a1 coord 

# 311 -> 279 unique loops
...
# 304 -> 248 unique promoters
# same promoter can be in same a2 coord (different myid_loop but same startJ)
# prom.in.Loops.a2.promoters <- gr_promoters[indices.p.a2@from]
...
#prom.in.Loops.a2.loops <- gr_loops_a2[indices.p.a2@to]
# 304 -> 273 unique loops
...
# any promoter and any core COMPLETE overlap 
...
saveRDS(a1prom.a2core.loops, "./output/R_objects/a1prom.a2core.loops.complete.overlap.rds")
saveRDS(a1core.a2prom.loops, "./output/R_objects/a1core.a2prom.loops.complete.overlap.rds")
...
dim(a1prom.a2core.loops) # 90 x 20
dim(a1core.a2prom.loops) # 111 x 20
...
# 106 x 3, all promoters in each row but may be mapped to same gene
...
genes.a1prom.a2core <- data.frame(genes.a1prom.a2core) # 106 x 5
...
# dim(genes.a1prom.a2core.uniq) # 78 x 17
...
saveRDS(genes.a1prom.a2core.uniq.fin, file = glue("./output/R_objects/", "genes.a1prom.a2core.uniq.fin.rds"))
saveRDS(genes.a1core.a2prom.uniq.fin, file = glue("./output/R_objects/", "genes.a1core.a2prom.uniq.fin.rds"))
write.xlsx2(genes.a1prom.a2core.uniq.fin, file = glue("./output/R_objects_files/", "genes_a1prom_a2core_uniq.xlsx"))
write.xlsx2(genes.a1core.a2prom.uniq.fin, file = glue("./output/R_objects_files/", "genes_a1core_a2prom_uniq.xlsx"))
